...
class LeNet:
    def __init__(self, dataset):
        ...
        self.dense_final = Layer_Dense(self.n_neurons, self.n_category)
    ...
    def run_training(self,
        ...
        for e in range(epochs):
            ...
            for i in range(iteration):
                ...
                self.ap1.forward(self.activation_tanh[0].output)
                
                self.conv2.forward(self.ap1.output, 0,1)
                self.activation_tanh[1].forward(self.conv2.output)
                self.ap2.forward(self.activation_tanh[1].output)
                
                self.conv2.forward(self.ap2.output, 2, 3)
                self.activation_tanh[2].forward(self.conv2.output)
